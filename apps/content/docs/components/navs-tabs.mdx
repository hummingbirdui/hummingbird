...
<div class="mb-5 alert alert-subtle-info items-start" role="alert">
  ...
  <span class="-mt-1">The base `.nav` component does not include an `.active` state. The examples show it for demonstration only. To indicate the active item for assistive technologies, use the `aria-current` attribute (`page` value for the current page or `true` for the current item in a set).</span>
</div>
...
Use the tab JavaScript plugin either individually or via `bootstrap.js` to extend tabs and pills into tabbable panes of local content.
