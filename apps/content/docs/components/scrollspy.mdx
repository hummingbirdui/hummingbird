...
To add scrollspy behavior to a topbar navigation, add `data-hb-spy="scroll"` to the element to be spied on (typically the `<body>`). Then, add the `data-hb-target` attribute with the `id` or class name of the parent element of any `.nav` component.
