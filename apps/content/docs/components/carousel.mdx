...
- The animation effects of this component are influenced by the prefers-reduced-motion media query. Refer to the reduced motion section within our accessibility documentation for further details.
...
## Methods
All API methods are asynchronous and initiate a transition. They return to the caller immediately after the transition begins but before it concludes. Furthermore, a method call on a component that is already transitioning will be disregarded. More details are available in our JavaScript documentation.
