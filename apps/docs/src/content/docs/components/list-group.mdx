...
The list group component is built using a set of CSS variables. These variables provide flexibility for customizing styles. Read more about customizing with CSS variables [here](/docs/customize/component#using-css-variables).
...
<Example showPreview={false} lang='sass' code={`.list-group {
    ...
    --list-group-hover-bg: var(--background-color-muted);
    ...
    --list-group-disabled-color: var(--text-color-subtle);
  }`} />