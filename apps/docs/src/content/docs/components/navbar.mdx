...
The Navbar component is built using a set of CSS variables. These variables provide flexibility for customizing styles. Read more about customizing with CSS variables [here](/docs/customize/component#using-css-variables).
...
<Example lang='sass' showPreview={false}
  code={`.navbar {
    ...
    --navbar-disabled-color: var(--text-color-subtle);
...
<HbAlert type="info">`--navbar-nav-link-padding-x` sets horizontal padding for links in the navbar's expanded state. `--nav-link-padding-x` sets horizontal padding for links in the navbar's collapsed state.</HbAlert>