...
<Example lang='sass' showPreview={false} code={`@import "tailwindcss";
  ...
  code={`@layer theme {
    ...
    :host {
      @variant dark {
        ...
        --border-color-muted: --alpha(var(--color-gray-700) / 80%);
        --border-color-default: var(--color-gray-700);
        --border-color-highlight: --alpha(var(--color-gray-600) / 80%);
