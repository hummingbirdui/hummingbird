...
On touch-enabled devices, opening a dropdown introduces temporary, empty mouseover handlers to the immediate children of the `<body>` element. This measure addresses an iOS event delegation quirk that would otherwise prevent a tap outside the dropdown from triggering the code responsible for closing it. Once the dropdown is closed, these additional mouseover handlers are removed.
...
Dropdowns require `data-bs-toggle="dropdown"` on their trigger element, regardless of whether they are called via JavaScript or the data-api.
