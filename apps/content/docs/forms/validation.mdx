---
title: Validation
description: Improve user experience by offering clear, actionable feedback through HTML5 form validation, using either native browser validation or custom styling and JavaScript for enhanced control.
toc: true
---

## Custom validation styles

Add the `novalidate` attribute to the `<form>` to disable the browser’s default feedback but still provides access to validation APIs in JavaScript. On submit, validation can prevent form submission and display feedback. Fields then show `:invalid` and `:valid` states, with custom colors, borders, focus styles.
