...
<Example code={`<div class="toast show" role="alert" aria-live="assertive" aria-atomic="true">
    ...
    <div class="border-t border-subtle py-2 px-3 flex justify-end gap-2">
...
<Example class="[&_.toast-container]:z-10" code={`<form>
...
<Example class='h-70 [&_.toast-container]:z-10' code={`<div aria-live="polite" aria-atomic="true" class="relative">
    <!-- Position it: -->
...
A list of all available utility classes for the toast component.
...
Toast component utilize several CSS variables for customization. Below is a list of available CSS variables that can be overridden to modify the appearance of Toasts. Read more about customizing with CSS variables [here](/docs/customize/component#using-css-variables).
...
<Example lang="js" showPreview={false} code={`const toastElList = document.querySelectorAll('.toast')
...
<Example showPreview={false} 
  code={`<button type="button" class="btn-close" data-bs-dismiss="toast" aria-label="Close"></button>`} 
/>

Alternatively, dismissal can be triggered from a button **outside the toast** by including the `data-bs-target` attribute:

<Example showPreview={false} 
  code={`<button type="button" class="btn-close" data-bs-dismiss="toast" data-bs-target="#my-toast" aria-label="Close"></button>`} 
/>
...
<HbTable>
  ...
  <tbody>
    <tr>
      ...
      <td scope="row">Applies a CSS fade transition to the toast.</td>
    </tr>
     <tr>
      <td scope="row">`autohide`</td>
      <td scope="row">boolean</td>
      <td scope="row">`true`</td>
      <td scope="row">Automatically hides the toast after the delay period.</td>
    </tr>
     <tr>
      <td scope="row">`delay`</td>
      <td scope="row">number</td>
      <td scope="row">`5000`</td>
      <td scope="row">The delay in milliseconds before the toast is automatically hidden.</td>
    </tr>
  </tbody>
</HbTable>
...
<HbTable>
  ...
  <tbody>
    <tr>
      ...
      <td scope="row">Hides an element’s toast. The toast element remains on the DOM but will no longer be shown.</td>
    </tr>
    <tr>
      <td scope="row">`getInstance`</td>
      <td scope="row">A static method that retrieves the toast instance associated with a DOM element.</td>
    </tr>
    <tr>
      <td scope="row">`getOrCreateInstance`</td>
      <td scope="row">A static method that retrieves the toast instance for a DOM element, or creates a new one if it wasn't initialized.</td>
    </tr>
    <tr>
      <td scope="row">`hide`</td>
      <td scope="row">Hides an element’s toast. This method must be called manually if `autohide` is set to `false`. Returns to the caller before the toast has actually been hidden.</td>
    </tr>
    <tr>
      <td scope="row">`isShown`</td>
      <td scope="row">Returns a boolean value indicating the toast's visibility state.</td>
    </tr>
    <tr>
      <td scope="row">`show`</td>
      <td scope="row">Reveals an element’s toast. This method must be called manually; otherwise, the toast will not be shown. Returns to the caller before the toast has actually been shown.</td>
    </tr>
  </tbody>
</HbTable>
...
<HbTable>
  ...
  <tbody>
    <tr>
      <td scope="row">`hide.bs.toast`</td>
      <td scope="row">This event is fired immediately when the `hide` instance method has been called.</td>
    </tr>
    <tr>
      <td scope="row">`hidden.bs.toast`</td>
      <td scope="row">This event is fired when the toast has finished being hidden from the user.</td>
    </tr>
    <tr>
      <td scope="row">`show.bs.toast`</td>
      <td scope="row">This event fires immediately when the `show` instance method is called.</td>
    </tr>
    <tr>
      <td scope="row">`shown.bs.toast`</td>
      <td scope="row">This event is fired when the toast has been made visible to the user.</td>
    </tr>
  </tbody>
</HbTable>

<Example lang="js" showPreview={false} 
  code={`const myToastEl = document.getElementById('myToast');
