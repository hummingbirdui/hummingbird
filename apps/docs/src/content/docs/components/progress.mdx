...
<Example code={`<div class="progress-stacked gap-0.5 h-3">
...
<Example class="flex items-center justify-center" code={`<div class="size-40">
    <svg class="size-full" viewBox="0 0 100 100">
      <circle
        class="stroke-current text-(--background-color-muted)"
        stroke-width="8"
        cx="50"
        cy="50"
        r="40"
...
A list of all available utility classes for the progress component.
...
The progress component is built using a set of CSS variables. These variables provide flexibility for customizing styles. Read more about customizing with CSS variables [here](/docs/customize/component#using-css-variables).
...
<Example lang='sass' showPreview={false}
  code={`.progress {
    ...
    --progress-bar-color: var(--color-inverse);
