---
title: Input Group
description: Input groups allow enhancing form controls by adding text, icons, buttons, or button groups to either side of an input, select, or file input.
toc: true
classNames:
  component:
    - class: input-group
...
Input groups can be sized using the `.input-group-sm`, `.input-group-lg`, default is medium. No need to add size classes to the individual elements, it's not supported.
...
Add checkboxes and radios as addons to input groups.
...
A list of all available utility classes for the input group component.
